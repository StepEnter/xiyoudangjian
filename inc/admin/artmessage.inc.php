...
if ($op=='display') {
    ...
    $list = pdo_fetchall("SELECT m.*,u.branchid,u.nickname,u.realname,u.mobile,u.headpic,a.title FROM ".tablename($this->table_artmessage)." m LEFT JOIN ".tablename($this->table_user)." u ON m.userid=u.id LEFT JOIN ".tablename($this->table_article)." a ON m.articleid=a.id ".$con." ORDER BY m.id DESC LIMIT ".($pindex-1) * $psize.",".$psize, $par);
    $total = pdo_fetch("SELECT count(m.id) as tol,u.branchid FROM ".tablename($this->table_artmessage)." m LEFT JOIN ".tablename($this->table_user)." u ON m.userid=u.id LEFT JOIN ".tablename($this->table_article)." a ON m.articleid=a.id ".$con ,$par);
    ...
    if ($_GPC['output']==1) {
        ...
        $arrhead = array("ID","组织ID","OpenID","昵称","姓名","学号","文章标题","留言信息","创建时间");
    ...
    $id = intval($_GPC['id']);
    $artmessage = pdo_fetch("SELECT m.*, u.branchid FROM ".tablename($this->table_artmessage)." m LEFT JOIN ".tablename($this->table_user)." u ON m.userid=u.id WHERE m.id=:id AND u.branchid IN (".$lbranchallid.") AND m.uniacid=:uniacid LIMIT 1 ", array(':id'=>$id,':uniacid'=>$_W['uniacid']));
    if (empty($artmessage)) {
        message_tip("你无权限删除该评论信息！", $this->createWebUrl('admin', array('r'=>'artmessage')), 'error');
    }
    pdo_delete($this->table_artmessage, array('id' => $id));
    message('评论信息删除成功！', referer(), 'success');

}
include $this->template('admin/artmessage');
