...
if ($op=='display') {
    ...
    $psize = 20;
    $con = " WHERE i.uniacid=:uniacid AND u.branchid IN (".$lbrancharrid.") ";
    ...
    if ($_GPC['output']==1) {
        ...
        $arrhead = array("ID","姓名","学号","积分类型","积分变动值","备注","创建时间");
    ...
    $con = " WHERE uniacid=:uniacid AND recycle=0 AND branchid IN (".$lbrancharrid.") ";
    $par[':uniacid'] = $_W['uniacid'];
    $branchid = intval($_GPC['branchid']);
    if ($branchid!=0) {
...
include $this->template('admin/integral');
