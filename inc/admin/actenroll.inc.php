...
if ($op=='display') {
    ...
    $psize = 50;
    $con = " WHERE e.uniacid=:uniacid AND a.branchid IN (".$lbrancharrid.") ";
    ...
    if ($_GPC['output']==1) {
        ...
        $arrhead = array("ID","组织ID","OpenID","昵称","姓名","学号","活动标题","报名类型","得分值","创建时间","签到时间");
    ...
    $activity = pdo_fetch("SELECT * FROM ".tablename($this->table_activity)." WHERE id=:id AND branchid IN (".$lbrancharrid.") AND uniacid=:uniacid LIMIT 1 ", array(':id'=>$actenroll['activityid'],':uniacid'=>$_W['uniacid']));
    if (empty($activity)) {
    ...
    $actenroll = pdo_get($this->table_actenroll,array('id'=>$id,'uniacid'=>$_W['uniacid']));
    if (empty($actenroll)) {
        message_tip('要删除的报名记录不存在或是已经被删除！', referer(), 'error');
    }
    $activity = pdo_fetch("SELECT * FROM ".tablename($this->table_activity)." WHERE id=:id AND branchid IN (".$lbrancharrid.") AND uniacid=:uniacid LIMIT 1 ", array(':id'=>$actenroll['activityid'],':uniacid'=>$_W['uniacid']));
    if (empty($activity)) {
        message_tip('你无权限操作该报名记录！', referer(), 'error');
    }
    pdo_delete($this->table_actenroll, array('id' => $id));
    message_tip('报名记录信息删除成功！', referer(), 'success');

}
include $this->template('admin/actenroll');
