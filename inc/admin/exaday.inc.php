...
if ($op == 'display') { 
    $con = " WHERE d.uniacid=:uniacid AND u.branchid IN (".$lbrancharrid.") ";
    ...
    if ($_GPC['output']==1) {
        ...
        $arrhead = array("ID","标题","状态","答对数目","答错数目","所得积分","姓名","学号","组织名称","完成时间","创建时间");
    ...
    $user = pdo_fetch('SELECT * FROM '.tablename($this->table_user).' WHERE id=:userid AND uniacid=:uniacid AND branchid IN ('.$lbrancharrid.') ', array(':userid'=>$exaday['userid'],':uniacid'=>$_W['uniacid']));
    if (empty($user)) {
...
include $this->template('admin/exaday');
?>