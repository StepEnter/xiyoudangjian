...
if ($op=='display') {
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($realname) || empty($mobile) || empty($idnumber)) {
            message('姓名、手机号和学号均不能为空！', referer(), 'error');
        }
        ...
        if (!empty($haveuser)) {
            message('手机号或学号已存在，请换个用户名或学号！', referer(), 'error');
        }
        $ulevel = intval($_GPC['ulevel']);
    ...
    if (!empty($haveuser)) {
        message('你要恢复的党员信息对应的手机号“'.$user['idnumber'].'”或学号“'.$user['mobile'].'”已存在，请检查目前已有党员信息！', '', 'error');
    }
