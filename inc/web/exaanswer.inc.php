...
if ($op == 'display') { 
    ...
    pdo_query("delete from ".tablename($this->table_exaitem)." WHERE itype=2 AND foreignid IN (".$idstr.")");
    $result = pdo_query("delete from ".tablename($this->table_exaanswer)." WHERE id IN (".$idstr.")");
    if (!empty($result)) {
        exit("success");
    }
    exit("error");
