...
if ($op=="display") {
    ...
    if (empty($mobile) && empty($idnumber)) {
        message("绑定登录时，学号和手机号不能同时为空！", referer(), 'error');
    }
	
	$user = pdo_get($this->table_user, $par);
	if (empty($user)) {
		message("该姓名、手机号和学号对应的党员信息不存在！", $this->createMobileUrl("login"), 'error');
	}
	if (!empty($user['openid'])) {
		message("该姓名学号已绑定过昵称为[".$user['nickname']."]的个人微信号，请先解除绑定！", referer(), 'error');
	}
	$data = array(
		'openid'     => $fan['openid'],
